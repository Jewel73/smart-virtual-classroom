...
import com.teamgreen.userservice.dto.AuthUserDto;
import com.teamgreen.userservice.dto.UserDto;
import com.teamgreen.userservice.request.RegisterRequest;
import com.teamgreen.userservice.request.UserUpdateRequest;
import com.teamgreen.userservice.service.UserService;
...
public class UserController {
    ...
    @PostMapping("/save")
    public ResponseEntity<RegisterRequest> save(@Valid @RequestBody RegisterRequest request) {    	
        return ResponseEntity.ok(modelMapper.map(userService.saveUser(request), RegisterRequest.class));
    }
    ...
    @PutMapping("/update")
    @PreAuthorize("hasRole('ADMIN') or hasRole('USER') or @userService.getUserById(#request.id).username == principal")
